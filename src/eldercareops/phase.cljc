(ns eldercareops.phase
  ...
  operations-coordination actor.
    ...
    Phase 3  supervised auto      -- governor-clean, high-confidence
                                     `:log-care-note`/`:schedule-family-visit`/
  ...
  needs a human to actually look at it. `eldercareops.governor`'s own
  `always-escalate-ops` enforces the same invariant independently --
  two layers, not one, agree on this."
  (:require [eldercareops.governor :as governor]))
...
(def phases
  ...
  {0 {:label "read-only"              :writes #{}                                                                           :auto #{}}
   1 {:label "assisted-logging"       :writes #{:log-care-note}                                                             :auto #{}}
   2 {:label "assisted-coordination"  :writes #{:log-care-note :schedule-family-visit :coordinate-supply-request
                                               :schedule-staff-shift-proposal}                                             :auto #{}}
   3 {:label "supervised-auto"        :writes write-ops
      :auto #{:log-care-note :schedule-family-visit :coordinate-supply-request :schedule-staff-shift-proposal}}})
