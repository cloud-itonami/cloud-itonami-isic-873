(ns eldercareops.governor-contract-test
  ...
  (testing "clean care-note proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1"
                               {:op :log-care-note :resident-id "resident-1" :patch {:meal "lunch" :mood "good"}}
                               ctx)]
      (is (some? result))
  ...
  (testing "every decision leaves immutable audit facts"
    (let [db (store/seed-db)
      ...
      (exec-request actor "t8a"
                     {:op :log-care-note :resident-id "resident-1" :patch {:meal "lunch"}}
                     ctx)
      (exec-request actor "t8b"
                     {:op :log-care-note :resident-id "unknown" :patch {:meal "lunch"}}
                     ctx)
